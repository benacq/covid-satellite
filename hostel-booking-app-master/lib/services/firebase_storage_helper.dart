...
//                      setState(() => _pathImage = val);
  ...
  Container getUploaded(double size, String headline2) {
    String subTitle = "";
    subTitle = _pathImage.contains("http://") || _pathImage.contains("https://")
        ? "Uploaded File"
        : "Selected: " + _pathImage.split('/').last;
    return Container(
      padding: EdgeInsets.all(5),
      child: Card(
        color: myAppTheme.cardColor,
        ...
        child: Column(
          children: <Widget>[
            ...
            Padding(
              padding: EdgeInsets.all(10),
              child: InkWell(
                  child: _pathImage.contains("http://") ||
                          _pathImage.contains("https://")
                      ? Image.network(
                          _pathImage,
                          width: 100,
                          height: 100,
...
//                      setState(() => _pathImage = val);
  ...
  Future<String> uploadDocuments(String filename, String fileExt) async {
    final StorageUploadTask uploadTask = FirebaseStorage.instance
        .ref()
        .child("" + "/" + filename + "." + fileExt)
        .putFile(File(_pathImage));
